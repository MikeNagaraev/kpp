...
public class Menu extends Application {
	...
	static NewGameInterface gi = new NewGameInterface(); // Declaration of new
																												// GamePlaying Process
	...
	public final static String MenuTitle = "F L A P P Y  B I R D";
	public final static String BirdMain = "birdmain.png";
	public final static String PetuhSound = "petuh.mp3";
	public final static String MainTheme = "maintheme.mp3";
	public final static String VolumePng = "volume.png";
	public final static String NotVolumePng = "notvolume.png";
	...
	@Override public void start(Stage primaryStage) throws Exception {
		...
		scene.getStylesheets()
				.add(getClass().getResource(style).toExternalForm());
	...
	public static Parent createContent() {
		...
		root.setStyle("-fx-background-color: #FFFF00"); // Add a Color YELLOW to
																										// Pane

		Rectangle rect = new Rectangle(W, H / 2); // Create rectangle for Dividing
																							// pane on 2 parts
		rect.setFill(Color.CYAN); // Add a Color BLUE to Pane
		rect.setTranslateX(0);
		rect.setTranslateY(H / 2);

		Title title = new Title(MenuTitle); // Creating title in Menu
		...
		SubMenu newGameMenu = new SubMenu(easy, normal, difficult,
				complexBack);

		MenuItem easyResult = new MenuItem(
				"BEST EASY :" + Rating.getResult(EASYMODE));
		MenuItem normalResult = new MenuItem(
				"BEST NORMAL :" + Rating.getResult(NORMALMODE));
		MenuItem difficultResult = new MenuItem(
				"BEST DIFFICULT :" + Rating.getResult(HARDMODE));
		MenuItem Back = new MenuItem("BACK");
		SubMenu statisticsMenu = new SubMenu();
		statisticsMenu.AddMenu(	easyResult, normalResult, difficultResult,
														Back);

		menuBox = new MenuBox(mainMenu);// Initial Setting a MainMenu

		// Setting a handler for special MenuItems
		...
		btn_volume.checkAction(); // Calling a method which check action with this
															// button
		...
		root.getChildren().addAll(rect, menuBox, title, btn_volume,
															volumeslider, getImageOfBird());
		return root;
	}
	...
	private static void checkOnstartGame(	MenuItem easy, MenuItem normal,
																				MenuItem difficult) {
	...
	private static ImageView getImageOfBird() {
		...
		try (InputStream is = Files.newInputStream(Paths.get(BirdMain))) {
			...
			imgMainBird.setOnMouseEntered(event -> {
				ScaleTransition st = new ScaleTransition(
						Duration.millis(rateOfScale), imgMainBird);
			...
			imgMainBird.setOnMouseClicked(event -> {
				...
				String path = PetuhSound;
				Media mediaSound = new Media(
						new File(path).toURI().toString());
				soundPlayer = new MediaPlayer(mediaSound);
				soundPlayer.setVolume(volume);
				soundPlayer.play();
				RotateTransition rt = new RotateTransition(
						Duration.millis(Rduration), imgMainBird);
	...
	private static class ButtonVolume extends Button {
		...
		public ButtonVolume() {
			try (InputStream is = Files
					.newInputStream(Paths.get(VolumePng))) {
		...
		public void LoadNotVolumePng() {
			try (InputStream is = Files
					.newInputStream(Paths.get(NotVolumePng))) {
				imgVolumeButton = new ImageView(new Image(is));
				imgVolumeButton.setFitHeight(sizeOfButton);
				imgVolumeButton.setFitWidth(sizeOfButton);
				setGraphic(imgVolumeButton);
